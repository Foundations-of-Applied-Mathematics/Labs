...
\objective{The finite element method is commonly used for numerically solving partial differential equations.
We introduce the finite element method via a simple BVP describing the steady state distribution of heat in a pipe as fluid flows through.}
...
To handle the large number of elements required for Problem \ref{prob:FEM_accuracy_comparison}, you will want to use sparse matrices from \li{scipy.sparse}.\footnote{See the Volume 1 lab ``Linear Systems'' for a refresher on sparse matrices.}

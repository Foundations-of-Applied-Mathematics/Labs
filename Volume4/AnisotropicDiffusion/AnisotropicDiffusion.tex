...
For this lab's examples we read in the image using the \li{imageio.v3.imread} function, and normalized it so that the colors are represented as floating point values between 0 and 1.
An image can converted to black and white when it is read by including the argument \li{mode='F'}.
...
Coding special cases for the edges and corners is tedious and error-prone, however.
...
So, if our original array \li{X} has shape \li{l,m}, then our padded array \li{Y} has shape \li{l+2,m+2}.
...
def anisdiff_bw(U, N, lambda_, g):
    ...
    pass
\end{lstlisting}
Run the function on \li{coke_balloon.jpg}.
Show the original image and the diffused image for $\sigma = .1$, $\lambda = .25$, $N = 5, 20, 100$.

Hint: Use the \li{np.pad} function to implement the padded array technique.
\end{problem}
...
To do this, we will define an iterative scheme similar to \eqref{eqn:anisotropic:diffuse_image}, but with $g$ having a different value at each pixel and for each timestep.
...
\end{split}
\end{align}
To compute the values of $g^n_{l,m}$, it is helpful to construct \li{(l,m)}-shape arrays of the $\|U^n_{l\pm1,m\pm1}-U^n_{l,m}\|$, put them together into a \li{(4,l,m)}-shape array $[\|U^n_{l-1,m}-U^n_{l,m}\|, \|U^n_{l+1,m}-U^n_{l,m}\|, \|U^n_{l,m-1}-U^n_{l,m}\|, \|U^n_{l,m+1}-U^n_{l,m}\|]$, and then using \li{np.argsort} with the argument \li{axis=0}.

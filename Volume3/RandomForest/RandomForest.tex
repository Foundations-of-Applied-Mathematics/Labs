...
To find the optimal split, begin by instantiating \li{best_gain} to 0 and \li{best_question} to \texttt{None}.
...
Instantiate a \li{Question} object with the column and value, and then use \li{partition()} to split the dataset into left and right partitions.
If either the left or right partition has less samples than the smallest allowable leaf size (called \li{min_samples_leaf}), then discard this partition and iterate to the next one.
If the left and right partitions are ok, then calculate the \li{info_gain()} of these two partitions with the Gini impurity of the dataset.
If this \li{info_gain()} is greater than \li{best_gain}, then set this \li{info_gain()} and its corresponding \li{Question} equal to \li{best_gain} and \li{best_question}, respectively.
Once you iterate through every column and row, return \li{best_gain} and \li{best_question}.

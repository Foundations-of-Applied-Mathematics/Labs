...
Test your function by running it on the gmmhmm given in the example, with $n\_sim=4$.
Print out the each state and its corresponding observations.

Note, $K$ is the same $K$ that defines the gmmhmm and is identified through the gmmhmm, so you don't need to set it.
\begin{comment}
Run the function 10 times and print out the states from each run.
...
\begin{problem}
Samples.zip contains 31 recordings for each of the words/phrases mathematics, biology, political science, psychology, and statistics. 

...
\begin{info}
You have likely been working very closely with JSON without even knowing it! Jupyter Notebooks are actually stored as JSON. To see this, open one of your \li{.ipynb} files in a basic text editor.
\end{info}
...
Python has a module in the standard library that allows easy manipulation of times and dates. The functionality is built around a \li{datetime} object.
...
We can use the \li{requests} library from the previous lab to \li{GET} the data from the page.
...
One efficient way of solving this problem is to perform a nearest neighbor search using a KDTree. We recommend you use scipy's \li{cKDTree} and its \li{query} method to find the nearest neighbor for each point.

Here's a quick example of how to use \li{cKDTree}.
...
The \li{random_newyork_locations.csv} file contains about 35,000 uniformly distributed points throughout New York City measured in longitude and latitude.
...
Therefore, use the \li{convert()} function in Problem 2 to transform your longitude and latitude points to epsg:3857. These units very closely approximately meters.

\lab{Finding Patterns in Data: LSI and more about Scikit-Learn}{LSI and SkLearn}
...
Applying the techniques of LSI discussed above to the word count matrix $X$, and keeping the first 7 principal components, write a function that takes in the path to a single State of the Union address \li{speech} and returns a tuple of the addresses that are most and least similar to \li{speech}.
...
Use the equation above to edit the function \li{weighted_document_converter()} to calculate the sparse matrix $A$.

...
With \li{ipyparallel} installed, we can now initialize an IPython cluster. 
...
\begin{problem}
Using \li{apply\_sync()}, draw 1,000,000 samples from a standard normal distribution. Print the mean, max, and min for draws on each individual engine. For example if you have four engines running, your output should look like:
\begin{lstlisting}
...
That is, time how long it takes to run the function on all of your machine's engines simultaneously using \li{apply\_sync}, and time how long it takes to run the function in a \li{for} loop $n$ times, where $n$ is the number of engines on your machine. 
Print the results.
\end{problem}

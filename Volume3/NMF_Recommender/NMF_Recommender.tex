...
Finally add a method called \texttt{reconstruct} that reconstructs and returns \texttt{V} by multiplying \texttt{W} and \texttt{H}. You may assume that \texttt{fit} has already been run (so \li{self.V} and \li{self.W} will already be initialized, in particular).
\end{problem}
...
\begin{problem}
Read the file \texttt{artist\_user.csv} as a pandas dataframe with \texttt{index\_col}\li{=0}.
...
\begin{enumerate}
  ...
  \item Next, iterate through \li{rank}$=10,11,12,13,\dots$ and for each iteration:
  \begin{enumerate}
    \item Run NMF using \texttt{n\_components}\li{=rank}, \texttt{init}\li{="random"}, \texttt{random\_state}\li{=0} and \texttt{max\_iter}\li{=500}
...
Use the file \texttt{artists.csv} with \texttt{index\_col}\li{=0}, to match the artist ID to their name.

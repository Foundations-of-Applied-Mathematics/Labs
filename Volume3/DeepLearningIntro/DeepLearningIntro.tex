...
% TODO: this one is probably a confusing explanation, but at least it explains it now
...
These often result from forgetting to specify the correct device for all components, including the model and data (using \li{variable.to(device)}).
...
To calculate the gradient of the model with respect to a piece of data \li{x}, we first need to set \li{x.requires_grad = True} and call \li{x.retain_grad()}.
% TODO: the second part might not actually be necessary; further testing needed

...
Use your function to predict the next $n$ values of the time series found in \li{weather.npy}.
...
As in Problem \ref{prob:model-identification}, perform model identification to find the ARMA($i,j$) model with the best AIC for $1\leq i \leq$ \li{p_max} and $1 \leq j \leq$ \li{q_max}, but this time use \li{statsmodels}.
Make sure the model is fit using the MLE.

Use the optimal model to predict $n$ future observations of the time series.
Plot the original observations along with the predicted observations from the beginning through $n$ observations in the future, as given by \li{statsmodels}.
...
Note that when fitting a VARMAX model, setting the parameter \li{ic='aic'} selects parameters based on AIC criterion. 

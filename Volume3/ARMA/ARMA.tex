...
\noindent Hint: you might find \li{np.diff()} to be usefull.
...
In this case, the time series should be the change in temperature of \li{weather.npy}, which is the first difference of the time series found in \li{weather.npy}, as was done in the first problem.
Return the log-likelihood of the $\text{ARMA}(p,q)$ model as a \li{float}.

Use the \li{state_space_rep()} function provided to generate $F,Q,$ and $H$.
The function \li{kalman()} has also been provided to calculate the means and covariances of each observation.

\noindent Hint: calling the function \li{kalman()} on a time series will return an array whose values are $x_{k\mid k-1}$ and an array whose values are $P_{k\mid k-1}$ for each $k\leq n$.
...
Write a function \li{model_identification()} that accepts \li{filename} containing the time series data and parameters $p_max$ and $q_max$ as integers. 
...
Write a function \li{arma_forecast()} that accepts \li{filename} containing a time series, the parameters for an ARMA model, and the number $n$ of observations to forecast.
...
\begin{problem}
Write a function \li{sm_arma()} that accepts \li{filename} containing a time series, integer values for \li{p_max} and \li{q_max}, and the number $n$ of values to predict.
Use \li{statsmodels} to perform model identification as in Problem \ref{prob:model-identification}, where the order of ARMA($i,j$) satisfies $1\leq i \leq$ \li{p_max} and $1 \leq j \leq$ \li{q_max}.
...
\caption{Statsmodel ARMA(3,3) forecast on \li{weather.npy}.}
\label{fig:sm}
\end{figure}

...
While subsets of data can be accessed in \li{DataFrames} using slicing, similar to a NumPy array, subsets can also be selected using the \li{iloc} and
\li{loc} indexers.
...
their labels, while the \li{iloc} method selects them based on their
integer position.
...
A column of a \li{DataFrame} may also be accessed using simple square
...
Additionally, pandas have functions such as \li{describe()}, \li{head()}, and \li{tail()} which can help to view and analyze data.
...
\begin{problem}
Answer the following questions using the file \li{crime_data.csv} and the pandas numerical methods.

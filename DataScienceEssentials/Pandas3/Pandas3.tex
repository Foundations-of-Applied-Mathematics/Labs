...
For part three, the answer should be a tuple where the first entry is the \li{Interval} of the age and the second is the \li{Interval} of the degree.
...
\begin{enumerate}
\item Partition \li{perc.alumni} into evenly spaced intervals of 20\% using \li{pd.cut()}.
...
\item Partition \li{Grad.Rate} into evenly spaced intervals of 20\% using \li{pd.cut()}.
Is the partition with the greatest number of schools the same for private and public universities?

\item Create a column that gives the acceptance rate of each university (using columns \li{Accept} and \li{Apps}).
Partition this column into evenly spaced intervals of 25\% using \li{pd.cut()}.

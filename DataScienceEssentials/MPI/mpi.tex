...
\lstinputlisting[style=fromfile]{examplecode/hello.py}
...
\begin{warn}
    Note that some systems may require \li{python3}, \li{py}, or a specific version like \li{python3.10} instead of \li{python} as shown above.
...
\begin{warn}
    ...
    In this case, the \li{--oversubscribe} flag must be used to ignore the number of availalbe slots when running multiple processes.
...
\end{description}
Example:
\lstinputlisting[style=FromFile]{examplecode/Get_size_example.py}
\item[Comm.Get\_rank()]
...
\end{description}
Example:
\lstinputlisting[style=FromFile]{examplecode/Get_rank_example.py}
\end{description}
...
\lstinputlisting[style=FromFile]{examplecode/separateCode.py}

\begin{problem}
Write a program which determines the rank \li{n} of the calling process and prints ``Hello from process \li{n}'' if \li{n} is even and ``Goodbye from process \li{n}'' if \li{n} is odd.
\end{problem}
...
\lstinputlisting[style=FromFile]{examplecode/passValue.py}
...
The \li{buf} object is not as simple as it appears. It must contain a pointer to a Numpy array. 
For example, a string must be packaged inside an array before it can be passed.
The \li{tag} object can help distinguish between data if multiple pieces of data are being sent/received by the same processes.
...
\end{description}
Example:
\lstinputlisting[style=FromFile]{examplecode/Send_example.py}
\end{description}

\begin{problem}
Write a script that runs on two (and only two!) processes and passes a random numpy array of length \li{n} from the root process to process 1.
Write it so that the user passes in the value of \li{n} as a command-line argument. 
...
n = int(argv[1])
\end{lstlisting}
In process 1, instantiate a zero array of length \li{n}, then print this array clearly labeled under process 1.
...
Then send the randomly generated array in the root process to process 1, and print the array clearly labeled under process 1.
    The output should reflect the following for \li{n} $ = 4$ (make sure to follow the output formatting exactly).
\begin{lstlisting}[style=ShellOutput]
...
\noindent Hint: if the number of processes is not $2$, you can abort the program with \li{COMM.Abort()}.
\end{problem}
...
\lstinputlisting[style=FromFile]{examplecode/pi.py}
\begin{lstlisting}
$ python pi.py
3.166
\end{lstlisting}

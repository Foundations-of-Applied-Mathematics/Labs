...
To see the all the contents of your current directory, type the command \li{ls} to "list" the files.
...
There should be a file called \texttt{Shell1.zip} and a script called \li{unixshell1.sh}.\footnote{If the necessary data files are not in your directory, \texttt{cd} one directory up by typing \texttt{cd ..} and type \footnotesize{\li{bash download_data.sh}} to download the data files for each lab.}
...
\begin{problem}
The computer with host name \li{acme20.byu.edu} contains a file that is called \li{img_649.jpg}.
Secure copy this file to your \li{UnixShell1/} directory.
(Do not add the \li{scp} command to the script).

To \li{ssh} or \li{scp} to this computer, your username is your Net ID, and your password is your typical Net ID password.
To use \li{scp} or \li{ssh} to access this computer, you will have to be on campus using BYU Wifi (or eduroam), or you can install a BYU specific VPN as explained at \url{https://it.byu.edu/it?id=kb_article&sys_id=32da18c2877ad110df3c635d0ebb3551}.
...
\noindent Hint: To use \li{scp}, you will need to know the location of the file on the remote computer.
...
This screen, displayed in \li{vim} for this example (\url{https://en.wikipedia.org/wiki/Vim_(text_editor)}), is asking you to enter a message to create a \emph{merge commit} that will reconcile both changes.

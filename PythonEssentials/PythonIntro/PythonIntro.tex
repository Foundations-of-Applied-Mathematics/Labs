...
To install Python via Anaconda, go to \url{https://www.anaconda.com/download/}, download the installer for \textbf{Python 3.7} corresponding to your operating system, and follow the on-screen instructions.
...
%Note that Python 2.7 is syntactically different than Python 3.7, and support for Python 2 will be phased out by the end of 2020.
...
Use the command \li{ls} to list the files and folders in the current directory, \li{pwd} to print the working directory, and \li{cd} to change directories.
...
Functions are defined with \emph{parameters} and are called with \emph{arguments}, although the terms are often used interchangeably.
...
Lambda functions cannot have custom docstrings, so the \li{lambda} keyword should only be used as a shortcut for very simple or intuitive functions that do not need additional labeling.
...
Python also supports computations of complex numbers by pairing two numbers as the real and imaginary parts.
...
Initialize a set with curly braces \li{\{ \}} and separate the values by commas, or use \li{set()} to create an empty set.
...
The \li{break} and \li{continue} statements also work in for loops. 
A \li{continue} in a for loop will automatically increment the index or item, whereas a \li{continue} in a while loop makes no automatic changes to any variable.
...
Since \li{wrapper()} accepts \li{*args} and \li{**kwargs} as arguments, the input function \li{func()} accepts any number of positional or keyword arguments.

...
<g<In [5]:>g> pts = np.random.random(10000)

# Time using evalf() on each of the random points.
<g<In [6]:>g> %timeit [expr.subs(x, pt).evalf() for pt in pts]
5.48 s +- 945 ms per loop (mean +- std. dev. of 7 runs, 1 loop each)

# Lambdify the expression and time using the resulting function.
<g<In [7]:>g> f = sy.lambdify(x, expr)

<g<In [8]:>g> %timeit [f(pt) for pt in pts]
2.5 ms +- 40.1 mms per loop (mean +- std. dev. of 7 runs, 100 loops each)

# Lambdify the expression with NumPy and repeat the experiment.
<g<In [9]:>g> f = sy.lambdify(x, expr, "numpy")

<g<In [10]:>g> %timeit f(pts)
98 mms +- 6.5 mms per loop (mean +- std. dev. of 7 runs, 10000 loops each)
\end{lstlisting}
\end{info}

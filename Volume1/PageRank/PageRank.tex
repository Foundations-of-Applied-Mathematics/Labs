...
Your answer should be a real number, so use \li{.real} to constrain the sum to the reals (it might try to have an imaginary part equal to 0).
...
\begin{unittest}
The file \li{test_pagerank.py} contains a prewritten unit test to test your \li{linsolve()} method for Problem 2.
There is a place for you to add unit tests to test your other two methods for Problem 2, \li{eigensolve()} and \li{itersolve()}, which will be graded.
\end{unittest}
...
\node[normalcircle] (D) [below of=C] {d};
% Weighted Edges
\foreach \i/\j/\w in {A/B/2,A/D/1,C/D/2}

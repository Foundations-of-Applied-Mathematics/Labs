...
\objective{Newton's method, the classical method for finding the roots of a function, is one of the most important algorithms of all time.
...
\emph{Newton's method} is an iterative method for finding the roots of a function.
...
\caption{
Newton's method approximates the root of a function (blue) by choosing as the next approximation the $x$-intercept of the tangent line (red) that goes through the point $(x_k, f(x_k))$.
In this example, $f(x) = e^x - 2$, which has a root at $\bar{x} = \log(2)$.
...
After only a few iterations, the root $\log(2)\approx 0.6931$ is already computed to several digits of accuracy.}
\label{fig:newton}
\end{figure}
...
Use Newton's method as described in (\ref{eq:newton-1d-def}) to compute a root $\bar{x}$ of $f$.
...
Check that the computed root $\bar{x}$ satisfies $f(\bar{x}) \approx 0$.
...
Newton's method can then be used to compute the roots of this function.
\end{info}
...
One potential problem occurs when the step from $x_k$ to $x_{k+1}$ is so large that the root is stepped over completely.
...
However, setting $\alpha=.4$, the iteration should converge to a root that is close to $0$.
\label{prob:newton-1d-backtracking}
\end{problem}

The backtracking constant $\alpha$ is significant, as it can result in faster convergence or convergence to a different root (see Figure \ref{fig:newton-backtracking-multi-result}).
...
Use Newton's method to find a root of $f$ using various values of $\alpha$ in the interval $(0,1]$.
...
Modify your function from Problems \ref{prob:newton-basic} and \ref{prob:newton-1d-backtracking} so that it can compute a root of a function $f:\mathbb{R}^n\rightarrow\mathbb{R}^n$ for any $n\in\mathbb{N}$.
...
When a function $f$ has many roots, the root that Newton's method converges to depends on the initial guess $x_0$.
...
Starting in one basin of attraction leads to finding one root, while starting in another basin yields a different root.
...
The basin for the root at the origin is connected, but the other two basins are disconnected and share a kind of symmetry.
...
Since choosing a different value for $\alpha$ can change which root Newton's method converges to, the basins of attraction may change for other values of $\alpha$.}
\end{figure}
...
In particular, Newton's method can be performed over the complex plane $\mathbb{C}$ to find imaginary roots of functions.
Plotting the basins of attraction over $\mathbb{C}$ yields some interesting results.

The roots of $f(x) = x^3 - 1$ are $1$, and $-\frac{1}{2} \pm \frac{\sqrt{3}}{2}i$.
...
After enough iterations, the $(i,j)$th element of the grid $X_k$ corresponds to the root of $f$ that results from using the $(i,j)$th element of $X_0$ as the initial point.
For example, with $f(x) = x^3 - 1$, each entry of $X_k$ should be close to $1$, $-\frac{1}{2} + \frac{\sqrt{3}}{2}i$, or $-\frac{1}{2} - \frac{\sqrt{3}}{2}i$.
Each entry of $X_k$ can then be assigned a value indicating which root it corresponds to.
...
Write a function that accepts a function $f:\mathbb{C}\rightarrow\mathbb{C}$, its derivative $f':\mathbb{C}\rightarrow\mathbb{C}$, an array \li{roots} of the roots of $f$, bounds $[r_{\text{min}},r_{\text{max}},i_{\text{min}},i_{\text{max}}]$ for the domain of the plot, an integer \li{res} that determines the resolution of the plot, and number of iterations \li{iters} to run the iteration.
...
To compute the $(i,j)$th entry $Y_{i,j}$, determine which root of $f$ is closest to the $(i,j)$th entry of $X_k$.
Set $Y_{i,j}$ to the index of this root in the array \li{roots}.
If there are $R$ distinct roots, each $Y_{i,j}$ should be one of $0,1,\ldots,R-1$.

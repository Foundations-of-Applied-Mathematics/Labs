...
\label{Lab:GaussQuad}
\labdependencies{PolynomialInterpolation}
...
\\(Note: The order of the points and weights in the table may differ depending on whether you used \li{scipy.linalg.eig()} or \li{scipy.linalg.eigh()}. The order doesn't matter but it is important that each point corresponds to the correct weight.)
\end{problem}

\begin{unittest}
In the file \li{test_gaussian_quadrature.py}, write unit tests for the \li{points_weights()} method in your \li{GaussianQuadrature} class. Some unit tests have been provided for Problem $1$. \\ \\
...
Test your method with examples that are easy to compute by hand and by comparing your results to \li{scipy.integrate.quad()} (The answer given below is found by using Chebyshev, even though the default mode is Lagrange.)
\begin{lstlisting}
>>> import numpy as np

...
\begin{warn}
	If you are having trouble with \li{pip install cvxpy} check the following:
	\begin{itemize}
...
Use \li{l1Min()} to verify that the solution which minimizes $||\mathbf{x}||_1$ is approximately $\x = [0., 2.571, 1.857, 0.]\trp $ and the minimum objective value is approximately $4.429$.
...
%Problem 6
 \begin{problem}
Read in the file \li{food.npy} (this data is pickled, so you'll need to pass the keyword argument \li{allow_pickle=True} into \li{np.load()}).

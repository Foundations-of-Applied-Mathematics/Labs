...
\labdependencies{LinkedLists}

\objective{Linked data structures chain data together in a useful way for many applications.
A tree is a linked data structure that can be used for efficient sorting and searching algorithms.
In this lab, we overview the basics of linked data structures and recursion.
...
\begin{figure}[H] % Singly linked list.
  ...
  \tikzset{rect node/.style={rectangle, draw, minimum height = .5cm, minimum width=.2cm}}
      ...
      \node[draw = none, black, node distance=1.4cm] [above left of=1](H) {\li{head}};
  ...
  \end{tikzpicture}
  \caption{A singly linked list with just a reference to the head node.}
...
\end{figure}
  ...
  \tikzset{rect node/.style={rectangle, draw, minimum height = .5cm, minimum width=.9cm}}
      ...
      \node[draw = none, black, node distance = 1.4cm] [above right of=4] (T) {\li{tail}};
      \node[draw = none, black, node distance = 1.4cm] [above left of=1] (H) {\li{head}};
      \node[rect node](1.5)[]{};
  ...
  \end{tikzpicture}
  \caption{A doubly linked list with references to both the head and tail nodes.}
  \label{fig:doublylinked}
\end{figure}

\subsection*{Modifying Linked Lists} % ------------------------------------------------

To insert a new node into a doubly linked list, we assign its \li{prev} and \li{<<next>>} attributes to reference the nodes that come before and after it.
...
\begin{figure}[H] % insert().
  ...
  \label{fig:doubly-linked-insert}
\end{figure}

\begin{figure}[H] % remove().
  \centering
  \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto, node distance=1.5cm,thick,main node/.style={rectangle,draw}, minimum size=.5cm]
  \tikzset{rect node/.style={rectangle, draw, minimum height = .5cm, minimum width=.2cm}}
      \node[main node] (1) {A};
  ...
  Python will automatically delete node C since there are no references to it.}
  \label{fig:doubly-linked-remove}
\end{figure}
...
\begin{problem}
  ...
  Add an \li{insert()} method to the \li{DoublyLinkedList} class that accepts an integer \li{index} and \li{data} to add to the list.
  Insert a new node containing \li{data} immediately before the node in the list at position \li{index}.
  ...
  If \li{index} is equal to the number of nodes in the list, then add the new node to the end of the list.
  Carefully account for the special cases of inserting before the first node and after the last node.
  Make sure to properly reassign the \li{head} or \li{tail} attribute to reflect the new structure of the list.
  ...
  Hint: The \li{__len__()} and \li{__str__()} methods have been adapted for this class.
  Use them in addition to the provided unit test for writing/debugging purposes.
  Also, attributes can be chained together for locating specific nodes (\li{<<self>>.head.<<next>>}, \li{<<self>>.prev.prev.value}, etc.)
...
Write a method called \li{recursive_find()} that does the same task as \li{iterative_find()}, but with the following recursive approach.
...
Start the recursion by calling this inner function on the \li{head} node.
...
The unit test is found in the file \li{test_binary_trees.py} and the function is called \li{test_bst_remove}.
...
Write a function to compare the build and search times of the \li{DoublyLinkedList} from Problem \ref{prob:doubly-linked-list-insert}, the \li{BST} from Problems \ref{prob:bst-insert} and \ref{prob:bst-remove}, and the \li{AVL} provided in the Additional Materials section.
...
\item Time (separately) how long it takes to load a new \li{DoublyLinkedList}, a \li{BST}, and an \li{AVL} with the $n$ items.
(Load your \li{DoublyLinkedList} by inserting each new value at the head.)
\item Choose 5 \textbf{random} items from the subset, and time how long it takes to find all 5 items in each data structure.
Use the \li{find()} method for the trees, but to avoid exceeding the maximum recursion depth, use the provided \li{iterative_find()} method from Problem \ref{prob:recursive-linked-list-find} to search the \li{DoublyLinkedList}.
\end{enumerate}

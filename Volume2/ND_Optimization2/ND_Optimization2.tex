\lab{Gradient Methods}{Gradient Methods}

\objective{Discuss Steepest Descent Methods and Conjugate Gradient Methods and understand when and why to use each}

\section*{Gradient Methods}

The gradient of a function points in the direction of steepest increase of that function. It follows that the opposite direction is the 
direction of steepest decrease, or \emph{descent}. The obvious candidate for the descent direction from some point $x_i$ is now simply
$-\nabla f(x_i)$. This procedure is known as the Method of Steepest
Descent. A simple version of Steepest Descent assumes a step size of one, checks that it is actually decreasing, and moves in that direction.
If the step size is too large, it halves the step size and tries again. It does this until the step size for which is actually decreasing
is below a certain tolerance.

\begin{problem}
Implement a basic Steepest Descent method.
Write a function \li{steepestDescent()} that accepts an initial
guess $x_0$, the derivative of a convex function, $Df$, and a default tolerance of .0001 as inputs.
Continue the algorithm until $\alpha$ is less than the tolerance.
Return the optimizer $x^*$.
\end{problem}

Steepest descent, however, can be very inefficient for certain problems: depending on the geometry
...
There are many possibilities for solving a linear system of equations, each method with its own set of pros and cons. We
will now explore the \emph{Conjugate-Gradient algorithm}, which is a method for solving large systems of equations where other methods,
...
To understand why this algorithm is much more effective than a simple steepest descent and how the algorithm chooses each descent direction,
we next discuss the idea of vector conjugacy.

\begin{figure}
\centering
\includegraphics[width=.7\textwidth]{steepVsConj.pdf}
...
\begin{figure}[H]
\centering
    \includegraphics[width=.7\textwidth]{logreg.pdf}
